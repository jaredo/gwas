...
parser = argparse.ArgumentParser(description='Converts shapeit2 haps to phased vcf')
...
parser.add_argument('--flip', action='store_const', const=True,default=False,dest='flip', help='flip alleles')
...
outfile = wopen(args.output+".vcf.gz")
samplefile.next()
samplefile.next()
sampleids = []
...
outfile.write(string.join(["#CHROM","POS","ID","REF","ALT","QUAL","FILTER","INFO","FORMAT"]+sampleids,"\t")+"\n")
infile = ropen(args.input + ".haps")
for linenum,row in enumerate(infile):
    data = row.split()
    gt = [data[idx]+"|"+data[idx+1] for idx in range(5,len(data),2)]
    if args.flip:    
        ref = data[4] 
        alt=data[3]
    else: 
    ...
    outfile.write(string.join([args.chrom,data[2],data[1],ref,alt,".\t.\t.\tGT"]+gt,"\t")+"\n")
